...
4. 개발일정
   ...
   추가개발내용 : 유닛 업그레이드, 몬스터와 보스 1종류 더 추가 총 20웨이브
...
6. MainScean에 등장하는 GameObject
   ...
   4. ButtonUi
      ...
      * 상호작용 : 마우스로 뽑기버튼을 클릭하면 유닛을 새로 생성한다. 유닛을 판매버튼 위에 놓고 마우스를 놓으면 판매가 가능하다
...
9. 아쉬운것
    ...
    * 유닛이 무슨등급인지 표시해주면 좋을 것 같다.
    * 다른 등급의 유닛끼리 겹치게 하면 위치가 서로 바뀌면 좋을것 같다.
    * 확률표를 게임내에 보여줘야할것 같다.

10. 수업에 대한 내용
   * 디자인 패턴을 알고 이를 사용하면 어떤점이 좋은지 알게되었다.
   * 게임 프레임워크 제작하여 다른 게임을 만들때도 이를 이용해서 쉽게 만들수 있다는것을 알게되었다.
   * 수정이 용이한 코드 작성해서 나중에 요구사항이 변경되거나 밸런스 패치를 할때 쉽게 하는 방법을 알게되었다.
